...
function renderReviews(items) {
  ...
  if (!items.length) {
    list.innerHTML =
      '<p class="reviews-empty">Nessuna recensione ancora. Scrivi la prima!</p>';
    return;
  }
    ...
    if (!user?.email) {
      ...
      return;
        ...
        const res = await fetch(`/api/storie/${storyId}/recensioni`, {
            ...
            body: JSON.stringify({ email: user.email, testo }),
        });
...
function bindBookmark(storyId) {
  ...
  const loadInitialBookmarkState = async () => {
    ...
    try {
      const meRes = await fetch(
        `/api/utenti/email/${encodeURIComponent(user.email)}`,
      );
      if (!meRes.ok) return;
  ...
  btn.addEventListener("click", async () => {
    ...
    alert(
      data.isFavorite ? "Aggiunto ai preferiti." : "Rimosso dai preferiti.",
    );
  });
}
  ...
  if (!id) {
    ...
    try {
        ...
        const url = user?.email
            ? `/api/storie/${id}?email=${encodeURIComponent(user.email)}`
            : `/api/storie/${id}`;
    ...
    document.getElementById("metaReadTime").textContent =
      `${Math.max(1, Math.ceil(words / 200))} min`;
        ...
        document.getElementById("storySummary").textContent =
            storia.descrizione || "Nessuna sinossi disponibile.";
